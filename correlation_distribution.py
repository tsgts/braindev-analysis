...
human_correlations = np.nan_to_num(np.loadtxt('allen_data/dev_human/human_cropped_corr_spearman_matrix.txt'))
...
# 	if len([i for i in row if i < -0.75]) > 20:
...
# 	if len([i for i in row if i < -0.7]) > 20:
...
# print(len(list(set(mouse_regulators) & set(binders))))

# print(len(human_regulators))
# print(len(list(set(human_regulators) & set(binders))))
