...
Mouse models are fundamental to the current understanding of neurodegenerative disorders and serve as a readily available base for experimentation in drug development. However, less than 10\% of drugs that have shown success in mouse-based preclinical trials show similar success in subsequent human clinical phases, which suggests that significant differences are present at the molecular level.  This study indirectly compared the developmental transcriptomes of mouse and human brains in order to further examine this divergence. Publicly-available ISH and RNA-seq data were compiled into species-specific datasets. A novel high-precision machine learning model projected each dataset onto a 2D manifold, and a density-based clustering algorithm was applied. Cross-organism adjusted rand index analysis revealed a high degree of divergence at the spatial and temporal levels, which disputes the results of several previous analyses. Immediate applications of this study include justification for the low success of mouse-derived treatments, a method of validation for future transgenic mouse models, and a robust process for deep analysis and visualization of sparse transcriptomic data.
...
\caption{Clustering consistency. \label{fig:cross_scatter_same}%
}
\end{center}
\end{figure}
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=\columnwidth]{figures/cross_matrix_comparison}
